...
def obtener_titulo_publicacion(url):
    ...
    cant_resenias = obtener_datos_etiqueta(soup, [('span', 'ui-pdp-review__amount')]).replace('(', '').replace(')', '').strip()
    if cant_resenias == 'No encontrado':
        cant_resenias = 0
    
    # Obtener precio
    precio = obtener_datos_etiqueta(soup, [('span', 'andes-money-amount__fraction')]).replace('.', '').strip()
    ...
    return titulo, puntaje, cant_resenias, precio, vendedor, categoria

def main(productos_urls):
    datos = []
    
    for producto, url in productos_urls:
        titulo, puntaje, cant_resenias, precio, vendedor, categoria = obtener_titulo_publicacion(url)
        datos.append([producto, url, titulo, puntaje, cant_resenias, precio, vendedor, categoria])
        print('Info extraida sobre producto: ', producto)
        time.sleep(1)  # Esperar 1 segundo entre cada solicitud para no sobrecargar el servidor
    
    # Crear un DataFrame con los datos obtenidos
    df = pd.DataFrame(datos, columns=['Producto', 'Link', 'Titulo', 'Puntaje', 'Cant reseñas', 'Precio', 'Vendedor', 'Categoria'])
